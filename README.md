...
```bash
npx wrangler d1 execute monthly_expenses --command "CREATE TABLE IF NOT EXISTS records (id INTEGER PRIMARY KEY AUTOINCREMENT, record_id TEXT, type TEXT, category TEXT, description TEXT, amount REAL, member TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);"
```
...
*   **響應式設計**：使用 Tailwind CSS，完美適配手機與電腦版面。
